...
function gameOver() {
  ...
  gameRoutes.forEach(function (route) {
    ...
    if (values.filter(function (value) {return value === humanSymbol; }).length === gridSize) {
      answer = "human";
    } else if (values.filter(function (value) {return value === computerSymbol; }).length === gridSize) {
      answer = "computer";
    } else if (allSquares.filter(function (square) {return getSquareVal(square) !== " "; }).length === allSquares.length) {
      answer = "draw";
    }
  });
...
function computerMove() {
  ...
  function scoreRoutes(routes) {
    ...
    function scoreRoute(route) {
      ...
      humanSquares = route.filter(function (square) {return getSquareVal(square) === humanSymbol; }).length;
      computerSquares = route.filter(function (square) {return getSquareVal(square) === computerSymbol; }).length;
